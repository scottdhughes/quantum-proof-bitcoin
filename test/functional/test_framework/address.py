...
ADDRESS_BCRT1_UNSPENDABLE = encode_segwit_address("bcrt", 0, bytes([0] * 32))
ADDRESS_BCRT1_UNSPENDABLE_DESCRIPTOR = f'addr({ADDRESS_BCRT1_UNSPENDABLE})'
# Coins sent to this address can be spent with a witness stack of just OP_TRUE.
ADDRESS_BCRT1_P2WSH_OP_TRUE = encode_segwit_address("bcrt", 0, sha256(CScript([OP_TRUE])))
...
def create_deterministic_address_bcrt1_p2tr_op_true(explicit_internal_key=None):
    ...
    if explicit_internal_key is None:
        assert address == "bcrt1p9yfmy5h72durp7zrhlw9lf7jpwjgvwdg0jr0lqmmjtgg83266lqsekaqka"
...
def address_to_scriptpubkey(address):
    ...
    if version in (111, 121):  # regtest/testnet pubkey hash plus legacy PQBTC fixtures
        return keyhash_to_p2pkh_script(payload)
    elif version in (196, 197):  # regtest/testnet script hash plus legacy PQBTC fixtures
