...
Generates a pqbtc.conf file in `share/examples/` by parsing the output from `pqbtcd --help`. This script is run during the
release process to include a pqbtc.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-pqbtc-conf.sh
```
