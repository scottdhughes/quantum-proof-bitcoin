...
3. All content of the data directory, except for `pqbtc.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `pqbtc.conf`        | User-defined [configuration settings](pqbtc-conf.md) for `pqbtcd` or `pqbtc-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `pqbtcd.pid`        | Stores the process ID (PID) of `pqbtcd` or `pqbtc-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `pqbtcd` or `pqbtc-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [pqbtc.conf](pqbtc-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`pqbtc-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).
...
| pqbtc.conf                                               | [Generated](../contrib/devtools/gen-pqbtc-conf.sh) configuration file     |
| bin/bitcoin                                                | Command-line tool for interacting with Bitcoin. Calls other binaries below. |
| bin/pqbtc-cli                                            | Tool for making node and wallet RPC calls.                                  |
| bin/pqbtc-qt                                             | Bitcoin node and wallet GUI                                                 |
| bin/bitcoin-tx                                             | Tool for creating and modifying transactions                                |
| bin/bitcoin-util                                           | Miscellaneous utilities                                                     |
| bin/pqbtc-wallet                                         | Bitcoin wallet tool                                                         |
...
| *libexec/test_pqbtc-qt*                                  | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `pqbtc-cli`, `pqbtcd`, and others |
...
- README and pqbtc.conf files are included in binary releases but not installed in source builds.
