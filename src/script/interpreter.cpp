...
static bool MatchExactPQSingleSigWitnessScript(const CScript& script)
{
...
static bool IsExactPQSingleSigWitnessStack(const std::vector<valtype>& stack, const CScript& exec_script)
{
    return stack.size() == 1 &&
           stack.front().size() == pqsig::SIG_SIZE &&
           MatchExactPQSingleSigWitnessScript(exec_script);
}
...
bool static IsActivePQPubKey(const valtype& vchPubKey)
{
    return pqsig::ClassifyPkScript(vchPubKey) == pqsig::PkScriptParseStatus::VALID_ACTIVE;
}

bool CheckSignatureEncoding(const std::vector<unsigned char> &vchSig, unsigned int flags, ScriptError* serror) {
    // Empty signatures remain valid placeholders for CHECK(MULTI)SIG failure
    // paths (e.g. NULLFAIL semantics).
    if (vchSig.size() == 0) {
        return true;
    }
    if (vchSig.size() == pqsig::SIG_SIZE) {
        return true;
    }
...
bool static CheckPubKeyEncoding(const valtype &vchPubKey, unsigned int flags, const SigVersion &sigversion, ScriptError* serror) {
    const bool is_pq_pubkey = IsActivePQPubKey(vchPubKey);
    if ((flags & SCRIPT_VERIFY_STRICTENC) != 0 && !is_pq_pubkey && !IsCompressedOrUncompressedPubKey(vchPubKey)) {
        return set_error(serror, SCRIPT_ERR_PUBKEYTYPE);
    }
    if ((flags & SCRIPT_VERIFY_WITNESS_PUBKEYTYPE) != 0 && sigversion == SigVersion::WITNESS_V0 && !is_pq_pubkey && !IsCompressedPubKey(vchPubKey)) {
...
bool GenericTransactionSignatureChecker<T>::CheckECDSASignature(const std::vector<unsigned char>& vchSigIn, const std::vector<unsigned char>& vchPubKey, const CScript& scriptCode, SigVersion sigversion) const
{
    ...
    if (vchSigIn.size() == pqsig::SIG_SIZE) {
        if (!IsActivePQPubKey(vchPubKey)) return false;

        // Witness sighashes need the amount.
        if (sigversion == SigVersion::WITNESS_V0 && amount < 0) return HandleMissingData(m_mdb);

        // PQ signatures on the pre-Taproot path are consensus-locked to SIGHASH_ALL.
    ...
    const uint256 sighash = SignatureHash(scriptCode, *txTo, nIn, nHashType, amount, sigversion, this->txdata, &m_sighash_cache);
    return VerifyECDSASignature(vchSig, pubkey, sighash);
}
...
static bool ExecuteWitnessScript(const std::span<const valtype>& stack_span, const CScript& exec_script, unsigned int flags, SigVersion sigversion, const BaseSignatureChecker& checker, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    const bool allow_pq_sig_stack_item{IsExactPQSingleSigWitnessStack(stack, exec_script)};
    ...
    for (const valtype& elem : stack) {
        if (elem.size() > MAX_SCRIPT_ELEMENT_SIZE && !(allow_pq_sig_stack_item && &elem == &stack.front())) {
            return set_error(serror, SCRIPT_ERR_PUSH_SIZE);
        }
    }
...
static bool IsExactPQReplacementWitnessScript(const valtype& script_bytes)
{
    ...
    return MatchExactPQSingleSigWitnessScript(script);
}
