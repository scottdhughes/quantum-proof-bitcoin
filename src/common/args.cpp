...
const char * const BITCOIN_CONF_FILENAME = "pqbtc.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\PQBTC
    //   new: C:\Users\Username\AppData\Local\PQBTC
    // macOS: ~/Library/Application Support/PQBTC
    // Unix-like: ~/.pqbtc
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "PQBTC";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "PQBTC";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/PQBTC";
#else
    // Unix-like
    return pathRet / ".pqbtc";
#endif
#endif
}
